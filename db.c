...
void enqueue(Queue* q, uint32_t page_num) {
...
uint32_t dequeue(Queue* q) {
    if (!q->front) return UINT32_MAX;
    QueueNode* temp = q->front;
    uint32_t page_num = temp->page_num;
...
ssize_t my_getline(char **lineptr, size_t *n, FILE *stream) {
  ...
  // If no buffer exists yet, create an initial one
  if (*lineptr == NULL || *n == 0) {
    *n = 128; // Start with a reasonable default size
  ...
  while (1) {
    ...
    if (c == EOF) {
      if (pos == 0) {
        return -1; // No input, EOF immediately
      }
      break; // Return what we read so far
    }

    // Ensure we have room for the next character + null terminator
    if (pos + 1 >= *n) {
      size_t new_size = *n * 2;
  ...
  return pos; // Number of characters read (excluding '\0')
}
typedef struct {
  char* buffer;
...
void print_tree(Pager* pager, uint32_t page_num, uint32_t indentation_level) {
  void* node = get_page(pager, page_num); uint32_t num_keys, child; switch (get_node_type(node)) { case (NODE_LEAF): num_keys = *leaf_node_num_cells(node); indent(indentation_level); printf("- leaf (size %d)\n", num_keys); for (uint32_t i = 0; i < num_keys; i++) { indent(indentation_level + 1); printf("- %d\n", *leaf_node_key(node, i)); } break; case (NODE_INTERNAL): num_keys = *internal_node_num_keys(node); indent(indentation_level); printf("- internal (size %d)\n", num_keys); if (num_keys > 0) { for (uint32_t i = 0; i < num_keys; i++) { child = *internal_node_child(node, i); print_tree(pager, child, indentation_level + 1); indent(indentation_level + 1); printf("- key %d\n", *internal_node_key(node, i)); } child = *internal_node_right_child(node); print_tree(pager, child, indentation_level + 1); } break; }
}
void print_tree_by_level(Pager* pager, uint32_t root_page) {
  if (root_page == UINT32_MAX) return;
    ...
    for (int i = 0; i < nodes_in_level; i++) {
      ...
      if (get_node_type(node) == NODE_LEAF) {
        ...
        for (uint32_t k = 0; k < num_keys; k++) {
          printf("%d", *leaf_node_key(node, k));
          if (k != num_keys - 1) printf(",");
        }
        printf("] ");
      } else { // NODE_INTERNAL
        uint32_t num_keys = *internal_node_num_keys(node);
        printf("[");
        for (uint32_t k = 0; k < num_keys; k++) {
          printf("%d", *internal_node_key(node, k));
          if (k != num_keys - 1) printf(",");
        }
        printf("] ");
        ...
        for (uint32_t k = 0; k < num_keys; k++) {
          enqueue(&q, *internal_node_child(node, k));
        }
        enqueue(&q, *internal_node_right_child(node));
      }
    }
    printf("\n");
    level++;
...
MetaCommandResult do_meta_command(InputBuffer* input_buffer, Table* table) {
  if (strcmp(input_buffer->buffer, ".exit") == 0) {
    ...
    printf("Tree:\n");
    print_tree_by_level(table->pager, table->root_page_num);
...
PrepareResult prepare_statement(InputBuffer* input_buffer,
  ...
  if (strcmp(input_buffer->buffer, "bulk_insert") == 0) {
    statement->type = BULK_INSERT;
    return PREPARE_SUCCESS;
  }
